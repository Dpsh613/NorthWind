...
export const products = pgTable("products", {
  ...
  // in cents bcs stripe polar and other payment platforms ask in cents
  priceCents: integer("price_cents").notNull(),
  currency: text("currency").notNull().default("usd"),
  imageUrl: text("image_url"),
  /** ImageKit `fileId` for deletes */
  imageKitFileId: text("image_kit_file_id"),
  // the product availability is true by default
...
export const checkoutSessions = pgTable("checkout_sessions", {
  id: uuid("id").defaultRandom().primaryKey(),
  // cascade means delete children if parent is deleted - so if user is deleted , delete all its session activities
  //restrict = dont delete the parent if any child still points at it
  userId: uuid("user_id")
    .notNull()
    .references(() => users.id, { onDelete: "cascade" }),
  polarCheckoutId: text("polar_checkout_id").unique(),
  // items list in that particular order
...
export const orders = pgTable("orders", {
  ...
  userId: uuid("user_id")
    .notNull()
    .references(() => users.id, { onDelete: "cascade" }),
  // when it is created , payment is in pending state - if paid- paid , failed as failed.
